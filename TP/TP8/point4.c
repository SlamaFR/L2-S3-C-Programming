...
void dessineTabPoint2(TabPoint t, int x, int y) {
    int i;
    for (i = 0; i < t.nbPoints - 1; i++) {
        Point p = t.tab[i].point;
        MLV_draw_filled_circle(p.x, p.y, i / 10, fabriqueCouleur(i));
        MLV_draw_line(x, y, p.x, p.y, MLV_rgba(255, 0, 0, 255 - i));
    }
}
